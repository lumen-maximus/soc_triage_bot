...
- Parse SOAR container artifacts for baseline enrichment data
...
This service consolidates all SOAR data parsing capabilities:
- Action/runbook extraction from case history (CaseArtifactHarvester)
- Baseline enrichment extraction from SOAR artifacts (formerly SOARDataExtractor)
"""
...
from ..models import Action, ActionType, Signal
...
class CaseArtifactHarvester:
    ...
    def extract_baseline_enrichments(signal: Signal) -> Dict[str, Dict[str, Any]]:
        ...
        This method consolidates functionality from the former SOARDataExtractor service.
        ...
        for artifact in artifacts:
            ...
            if any(
                kw in name
                for kw in ["threat", "virustotal", "reputation", "ti_", "intelligence"]
            ):
                ...
                baseline["cmdb"].update(
                    {
                        ...
                        "soar_criticality": data.get("criticality")
                        or data.get("business_criticality"),

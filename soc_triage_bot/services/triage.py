...
class TriageService:
    ...
    async def triage(
        ...
        """Legacy triage method.

        DEPRECATED: Use triage_extended() for new code with multi-track support.
        ...
        # Step 2: Legacy single-track forecasting (if enabled)
        forecast_data = None
        if forecast_enabled and historical_data:
        ...
        # Step 3: Similar case retrieval (legacy)
        similar_cases = self.similarity_service.find_similar(signal)

        # Step 4: Classification (legacy)
        ...
        # Step 6: Generate report (legacy - would fail with new template)
        # For legacy calls, we need to assemble a minimal TriageReport
        try:
            triage_report = self._assemble_minimal_triage_report(
                signal, enrichments, classification, forecast_data, similar_cases
            )
            report = self.report_service.generate_report(triage_report)
        except Exception:
            report = (
                f"# Triage Report - {signal.signal_id}\n\nReport generation failed."
            )
    ...
    def _assemble_minimal_triage_report(
        self,
        signal: Signal,
        enrichments: Dict[str, EnrichmentResult],
        classification: Classification,
        ...
        """Assemble minimal TriageReport for legacy triage method."""
        from ..models import ClassificationLabel
        ...
        if classification.label == ClassificationLabel.TRUE_POSITIVE:
            disposition = "Likely True Positive"
        elif classification.label == ClassificationLabel.FALSE_POSITIVE:
        ...
        classification_result = ClassificationResult(
            ...
            tp_likelihood=classification.confidence,
            severity=signal.severity,
            confidence="medium",
            reasons_tp=classification.reasoning,
            reasons_fp=[],
            triage_judgment=f"Assessed as {disposition}",
        )

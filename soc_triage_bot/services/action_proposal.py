...
class ActionProposalService:
    ...
    def _load_templates(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "conditions": {
                    "classification": [ClassificationLabel.TRUE_POSITIVE, ClassificationLabel.UNKNOWN],
                    "min_confidence": 0.5,
    ...
    def _matches_conditions(
        ...
        if required_classifications and classification.label not in required_classifications:
            return False
        ...
        for entity_type, entity_values in signal.entities.items():
            ...
            if placeholder in description:
                description = description.replace(
                    placeholder, 
                    entity_values[0] if entity_values else entity_type
                )
    ...
    def _generate_learned_actions(
        ...
        if (classification.label == ClassificationLabel.TRUE_POSITIVE and 
            classification.confidence > 0.85):
            
            actions.append(Action(
                action_id=self._generate_action_id("learned-notify", signal.signal_id),
    ...
    def _generate_dynamic_actions(
        ...
        if vuln_result and vuln_result.status.value == "success":
            ...
            if critical_vulns > 0:
                actions.append(Action(
                    action_id=self._generate_action_id("gen-vuln", signal.signal_id),
